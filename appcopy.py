...
class UAVNetworkSimulation:
    ...
    #     for node in G.nodes():
    #         # Randomly increase latency by up to 10ms and decrease throughput by up to 10 Mbps
    #         G.nodes[node]["latency"] += random.randint(100, 150)
    #         G.nodes[node]["throughput"] = max(
    #             1, G.nodes[node]["throughput"] - random.randint(8, 15)
    #         )
    
    def update_network_ddos_metrics(self, G, target_ids):
        for target_id in target_ids:
            if target_id in G.nodes():
                print("nodes found")
            # Randomly increase latency by up to 10ms and decrease throughput by up to 10 Mbps
                G.nodes[target_id]["latency"] += random.randint(100, 150)
                G.nodes[target_id]["throughput"] = max(
                    1, G.nodes[target_id]["throughput"] - random.randint(8, 15)
                )
    ...
    def run_ddos_simulation(
        ...
        while time.time() - start_time < total_time:
            ...
            # Trigger DDoS attack
            if iteration == 1:
                self.update_network_ddos_metrics(G,target_ids)
